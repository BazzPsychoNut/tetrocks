...
GameEngineClass = Class.extend({
	...
	// array to keep track of which grid tiles are filled 
	grid: [],
	...
	reset_grid: function()
	{
		...
		// add starting area tiles to grid
	...
	move: function(x, y)
	{
		...
		{
			game.clear_tiles(game.block.tiles);
	...
	/**
	 * check if all tiles of this block can be placed at the given location
	 * @param x
	 * @param y
	 * @returns {Boolean}
	 */
	can_be_placed_at: function(x, y)
	{
		...
		{
			var tile_x = game.block.tiles[i].x + x;
			var tile_y = game.block.tiles[i].y + y;
	...
	{
		// loop through all tiles in the block and remove them from the grid
		for (var i=0; i<game.block.tiles.length; i++) 
		{
			var tile_x = game.block.tiles[i].x + game.block_location.x;
			var tile_y = game.block.tiles[i].y + game.block_location.y;
	...
	{
		// delete all tiles passed in tiles
		for (var i=0; i<tiles.length; i++) {
			tile = tiles[i];
			// check all tiles in the block and remove the one with the same coordinates
			for (var j=0; j<game.block.tiles.length; j++) {
				if (game.block.tiles[j].x == tile.x && game.block.tiles[j].y == tile.y) {
					game.block.tiles.splice(j, 1);
	...
	{
		var blobs = []; // collect in array, to not alter the game.block.tiles array while we are looping through it.
		
		// loop through all tiles in the block and place them on the grid
		for (var i=0; i<game.block.tiles.length; i++) 
		{
			var tile = game.block.tiles[i]; 
			...
			if (game.grid[tx][ty].blob) {
				...
				board.draw_tile(game.block.color, tile.special, tx, ty);
			}
				
			game.grid[tx][ty] = {"color": game.block.color, "special": tile.special, "x":tx, "y":ty};
		}
		
		// remove blob tiles from block
		if (blobs.length) 
	...
	turn: function(direction)
	{
		...
		var current_tiles = game.block.tiles;
		...
		// test if new tiles will fit
		game.block.tiles = tiles;
		...
		else
		{
			...
			game.block.tiles = current_tiles;
			game.place_on_grid();
			sfx.play('error');
		}
	},
	...
	/**
	 * remove tiles of block from board
	 */
	clear_tiles: function(tiles)
	{
		// loop through all tiles in the block and clear them from the board
		for (var i=0; i<tiles.length; i++) 
		{
	...
	put_down: function()
	{
		...
		if (found.length > 0) {
			...
			// calculate score before removing any tiles ;)
	...
	remove_tile: function(tile, with_bomb)
	{
		...
		if (tile.special && ! with_bomb) 
		{
			if (tile.special == 'brick') {
				...
				tile.special = null; 
				board.draw_tile(tile.color, tile.special, tile.x, tile.y); // draw tile without shield
				return null;
			}
		}
	...
	/**
	 * remove the given tiles from the board and grid and collect the specials to execute
	 * @param array tiles
	 */
				...
				for (var x=bomb.x-2; x<=bomb.x+2; x++) {
					for (var y=bomb.y-2; y<=bomb.y+2; y++) {
						...
						if (! game.grid[x] || ! game.grid[x][y]) // make sure the grid coordinates exist
							continue;
						if (game.grid[x][y].color || game.grid[x][y].blob) // only collect tiles with block or blob on them
	...
	{
		if (game.next_block.tiles)  // the first time there is not a previous next_block
		{
